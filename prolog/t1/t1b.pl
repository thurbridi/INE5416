...
% -----Funções----- %
    ...
    % Questão 1
	nomecompleto(Codigo, Nome) :- materia(Codigo, Nome, _, _).

    % Questão 2
	precomum(CodigoA, CodigoB, Requisito) :- prereq(CodigoA, Requisito), 
											 prereq(CodigoB, Requisito).

    % Questão 3
	prepre(Codigo, PrePreRequisito) :- prereq(Codigo, X), prereq(X, PrePreRequisito).
    ...
    % Questão 6
	temprecomumsaopre(Fase, CodigoA, CodigoB) :- fase(CodigoA, Fase), 
												 fase(CodigoB, Fase), 
												 precomum(CodigoA, CodigoB, _),
												 saopre(_, CodigoA),
												 saopre(_, CodigoB).

    % Questão 7
	proposta(Fase, Disciplina) :- materia(Disciplina, _, Fase, X), X=[].